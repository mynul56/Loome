...
import { ShoppingBag, Package, Clock, CheckCircle2, ChevronRight } from 'lucide-react';
...
const MyOrders: React.FC = () => {
  ...
  if (isLoading) {
    return <Layout><FootballLoader text="Fetching History..." /></Layout>;
  }
      ...
      <div className="bg-gray-50 min-h-screen pb-20">
        <div className="bg-black text-white px-6 lg:px-12 py-12 border-b-8 border-primary">
          <div className="max-w-[1400px] mx-auto">
            ...
            <p className="text-gray-400 font-bold uppercase tracking-widest mt-2">Track your 2026 squads</p>
          </div>
        </div>
        ...
        <div className="px-6 lg:px-12 max-w-[1400px] mx-auto mt-12">
          {orders.length === 0 ? (
            <div className="bg-white p-12 text-center border-4 border-black sport-shadow">
              ...
              <h2 className="text-3xl font-heading uppercase mb-4">No Orders Yet</h2>
              <p className="text-gray-500 font-bold mb-8 uppercase">You haven't secured any kits yet.</p>
              <Link to="/shop" className="inline-block bg-primary text-black px-12 py-4 font-heading text-2xl uppercase border-4 border-black hover:bg-black hover:text-white transition-all transform hover:-translate-y-1">
                Start Shopping
              </Link>
            </div>
          ) : (
            <div className="space-y-8">
              {orders.map((order) => (
                <div key={order.id} className="bg-white border-4 border-black sport-shadow overflow-hidden group">
                  {/* Header */}
                  <div className="bg-black text-white p-6 flex flex-wrap items-center justify-between gap-4">
                    <div className="flex items-center gap-6">
                      <div>
                        ...
                        <p className="font-heading text-xl text-primary">{order.id}</p>
                      </div>
                      <div className="hidden sm:block">
                        <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Date</p>
                        <p className="font-bold uppercase text-sm">{new Date(order.createdAt).toLocaleDateString()}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
                      <div className={`px-4 py-1 font-heading text-lg uppercase skew-x-[-12deg] flex items-center gap-2
                        ${order.status === 'Delivered' ? 'bg-green-600' : 
                          order.status === 'Cancelled' ? 'bg-red-600' : 'bg-primary text-black'}`}>
                    ...
                    <div className="lg:col-span-8 space-y-4">
                      {order.items.map((item, idx) => (
                        <div key={idx} className="flex gap-4 items-center border-b border-gray-100 last:border-0 pb-4 last:pb-0">
                          ...
                          <div>
                            <h4 className="font-heading text-xl uppercase leading-tight">{item.productName}</h4>
                            <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">
                              SIZE {item.selectedSize} • QTY {item.quantity}
                            </p>
                          </div>
                          <div className="ml-auto font-jersey text-xl">৳{item.price * item.quantity}</div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="lg:col-span-4 bg-gray-50 p-6 border-l-4 border-black flex flex-col justify-between">
                      <div className="space-y-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs font-bold uppercase">{order.phone}</p>
                        </div>
                      </div>
                      
                      <div className="mt-6 pt-6 border-t-2 border-gray-200">
                        <div className="flex justify-between items-end">
                          <span className="font-heading text-xl uppercase">Total Paid</span>
                          <span className="font-jersey text-4xl text-primary font-bold bg-black px-3">৳{order.totalPrice}</span>
                        </div>
                      </div>
                    </div>
                  </div>
