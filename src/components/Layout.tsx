...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col font-body selection:bg-primary selection:text-black relative">
      ...
      {/* Cart Drawer Panel */}
      <div
        className={`fixed top-0 right-0 h-full w-full max-w-[450px] bg-white z-[101] shadow-2xl transition-transform duration-500 transform ${isCartOpen ? "translate-x-0" : "translate-x-full"} flex flex-col border-l-8 border-black`}
      >
        <div className="bg-black text-white p-6 flex items-center justify-between border-b-4 border-primary">
          <h2 className="text-3xl font-heading uppercase tracking-wider">
            Your <span className="text-primary">Kits</span>
          </h2>
          <button
            onClick={() => setIsCartOpen(false)}
        ...
        <div className="flex-grow overflow-y-auto p-6 custom-scrollbar space-y-6">
          {cartItems.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center space-y-4 py-20">
              ...
              <p className="font-heading text-2xl uppercase text-gray-400">
                Empty Pitch
              </p>
              <Link
                to="/shop"
                className="bg-black text-white px-6 py-2 font-bold uppercase hover:bg-primary hover:text-black transition-colors"
              >
                Go To Shop
              </Link>
            </div>
            ...
            cartItems.map((item) => {
              ...
              return (
                <div
                  key={imageKey}
                  className="flex gap-4 border-b-2 border-gray-100 pb-6 relative group"
                >
                  ...
                  <div className="flex-grow flex flex-col justify-between py-1">
                    <div>
                      <h3 className="font-heading text-xl uppercase leading-tight mb-1">
                        {item.name}
                      </h3>
                      <p className="text-[10px] font-bold uppercase text-gray-500 tracking-widest">
                        {item.team} • SIZE {item.selectedSize}
                      </p>
                    </div>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center border-2 border-black">
                        ...
                        <span className="w-10 text-center font-jersey text-xl border-x-2 border-black">
                          {item.quantity}
                        </span>
                        <button
                      ...
                      <span className="font-jersey text-2xl font-bold">
        ...
        {cartItems.length > 0 && (
          <div className="p-6 bg-gray-50 border-t-4 border-black">
            <div className="flex justify-between items-end mb-6">
              ...
              <span className="font-jersey text-4xl font-bold text-black leading-none">
      ...
      <header className="sticky top-0 w-full z-50 bg-black text-white border-b-4 border-primary">
        <div className="w-full px-4 lg:px-8 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-8">
            <Link
              to="/"
              className="text-3xl font-heading tracking-wider uppercase flex items-center gap-2"
            >
          ...
          <div className="hidden lg:flex items-center gap-6 font-bold text-sm uppercase">
            {currentUser ? (
              ...
              <div className="flex items-center gap-4">
                ...
                <Link
                  to="/register"
                  className="bg-primary text-black px-4 py-2 rounded font-bold hover:bg-white transition-colors"
                >
                  Join
                </Link>
              </div>
        ...
        {isMenuOpen && (
          <div className="lg:hidden absolute top-20 left-0 w-full bg-secondary border-b-4 border-primary shadow-2xl flex flex-col font-heading text-2xl uppercase tracking-wider">
            <Link
              to="/shop"
              className="p-6 border-b border-gray-800 hover:text-primary hover:bg-black"
            >
              Kits 26
            </Link>
            <Link
            ...
            <Link
              to="/live"
              className="p-6 border-b border-gray-800 hover:text-primary hover:bg-black flex items-center gap-3"
            >
            ...
            {currentUser ? (
              <>
                {currentUser.role === "admin" && (
                  <Link
                    to="/admin/dashboard"
                    className="p-6 border-b border-gray-800 text-red-500 hover:bg-black"
                  >
                    Admin Panel
                  </Link>
                )}
                ...
                <button
                  onClick={handleLogout}
                  className="p-6 text-left border-b border-gray-800 hover:text-primary hover:bg-black w-full"
                >
                  Logout
                </button>
              </>
              ...
              <>
                ...
                <Link
                  to="/register"
                  className="p-6 text-primary hover:bg-black"
                >
                  Join Us
                </Link>
              </>
        ...
        <div className="w-full px-4 lg:px-8 max-w-[1800px] mx-auto grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          <div>
            ...
            <p className="text-gray-400 text-sm font-medium">
              Premium Authentic World Cup Gear. Built for the pitch, styled for
              the streets.
            </p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-gray-400 text-sm font-medium mb-4">
              Get exclusive drops and match day offers.
            </p>
            <div className="flex">
              <input
                type="email"
              ...
              />
              <button className="bg-primary text-black font-bold px-4 py-2 hover:bg-white transition-colors">
                JOIN
              </button>
            </div>
          </div>
